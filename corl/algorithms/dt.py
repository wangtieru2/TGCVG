...
def attach_generated_rtg(reward_array, rtg_init, guidance_target_multiple=1.2):
    temp = np.zeros_like(reward_array)
    temp[1:] = reward_array[:-1]
    rewards_cumsum = np.cumsum(temp)
    return np.ones_like(reward_array) * rtg_init * guidance_target_multiple - rewards_cumsum
    ...
    for subtrajectory in dataset:
        if 'RTG' in subtrajectory.keys():
            traj.append({
                ...
                'returns': attach_generated_rtg(subtrajectory['rewards'], subtrajectory['RTG'][0]),
    ...
    for step in trange(config.max_timesteps, desc="Training"):
        ...
        if step % config.eval_every == 0 or step == config.max_timesteps - 1:
            ...
            for target_return in config.target_returns:
                ...
                print("---------------------------------------")
                print(
                    f"Evaluation over target return {target_return}: "
