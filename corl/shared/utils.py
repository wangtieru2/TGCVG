...
def get_TGCVG_dataset(env: str, step: Optional[int]=None) -> Dict:
    data = np.load(f'xx.npz', allow_pickle=True) 
...
def get_dataset(config):
    if config.GDA is None or config.GDA == 'None':
        ...
        return dataset, {}
    elif 'TGCVG' in config.GDA:
        print("TGCVG data augmentation")
        generated_dataset, metadata = get_TGCVG_dataset(config.env)           
    else:
        raise RuntimeError("GDA must be one of the 'TGCVG' or 'None'.")
...
def get_trajectory_TGCVG_dataset(config):
    metadata = {}
    env = config.env
    if config.GDA is None or 'None' in config.GDA:
        with open(f'xx.pkl','rb') as f:
