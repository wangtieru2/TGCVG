...
class TransformerTrajectoryDataset(DiffusionDataset):
    def __init__(
        ...
        penalty=100 if penalty is None else penalty
        self.penalty = penalty
        self.dataset, info = self._load_d4rl_trajectories(dataset)
        self.trajectory_rewards = info["trajectory_rewards"]
    ...
    def _load_d4rl_trajectories(self, dataset: Dict) -> Tuple[List[DefaultDict[str, np.ndarray]], Dict[str, Any]]:
        traj, traj_len = [], []
        terminal_idx = []
        terminal_cnt = 0
        non_terminal_cnt = 0
        for episode_data in tqdm(dataset) :  
            ...
            if episode_data["rewards"].ndim == 1:          
                episode_data["rewards"] = episode_data["rewards"][..., None]
            ...
            if "antmaze" in self.dataset_nm:
                episode_data["rewards"] = (episode_data["rewards"]-0.5) * 4.0
        ...
        for tr, tlen in zip(traj, traj_len):
            for traj_idx in range(tlen - self.seq_len + 1):
                ...
                trajectory_rewards.append(trajectory_reward) 
                subtrajectories.append([states, actions, rewards, next_states, terminals, time_steps, rtg])  
